...
def main():
    ...
    with torch.no_grad():
        ...
        words_count = 0
        bits_count = 0
        with open('stego/' + DATASET + '/adg_pos.txt', 'w', encoding='utf8') as f:
            for sentence in stega_text:
                words_count += len(sentence)
                f.write(' '.join(sentence) + '\n')
        with open('stego/' + DATASET + '/adg_pos.bit', 'w', encoding='utf8') as f:
            for bits in stega_bits:
                bits_count += len(bits)
                f.write(bits + '\n')
        with open('stego/' + DATASET + '/adg_pos.txt', 'a', encoding='utf8') as f:
            f.write(" bpw:" + str(bits_count / words_count))
