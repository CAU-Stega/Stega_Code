...
def main():
	...
	# ======================
	os.makedirs('stego1/' + DATASET, exist_ok=True)
	# read bit streams
	...
	with torch.no_grad():
			...
			bits_counts = 0
			# write files
			with open('stego2/' + DATASET + '/adg_test.txt', 'a', encoding='utf8') as f:
				for sentence in stega_text:
					words_counts += len(sentence)
					f.write(' '.join(sentence) + '\n')
			# with open('stego2/' + DATASET + '/adg_pos.bit', 'a', encoding='utf8') as f:
			# 	for bits in stega_bits:
			# 		bits_counts += len(bits)
			# 		f.write(bits + '\n')
			# with open('stego2/' + DATASET + '/adg_pos.txt', 'a', encoding='utf8') as f:
			# 	f.write("bpw: " + str(bits_counts / words_counts))
